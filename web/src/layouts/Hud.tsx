...
const Hud: React.FC = () => {
    ...
    const [armour, setArmour] = useState<number>(100)
    ...
    useNuiEvent('setStatuses', (data: HudProps) => {
        ...
        if (data.statuses !== undefined) {
            ...
            if (status.hunger !== undefined) {
                ...
                if (status.hunger !== hunger) setHungerColor(status.hunger > 10 ? 'yellow' : 'red')
            }
            if (status.thirst !== undefined) {
                setThirst(status.thirst)
                if (status.thirst !== thirst) setThirstColor(status.thirst > 10 ? 'cyan.5' : 'red')
            }
            if (status.stress !== undefined) {
                setStress(status.stress)
                if (status.stress !== stress) setStressColor(status.stress < 50 ? 'orange' : 'red')
            }
            if (status.drunk !== undefined) {
                setDrunk(status.drunk)
                if (status.drunk !== drunk) setDrunkColor(status.drunk < 50 ? 'grape' : 'red')
            }
        }
    })
    ...
    return (
        ...
        {/* HEALTH */}
        {health < 100 && <RingProgress sections={[{ value: health, color: healthColor }]} thickness={6} size={55} roundCaps
            label={
            <Center>
